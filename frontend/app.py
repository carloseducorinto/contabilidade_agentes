        ...
        with col2:
            ...
            if health.get('status') == 'healthy':
                ...
                with st.expander("ℹ️ Informações da API"):
                    ...
                    with col_a:
                        st.metric("OpenAI", "✅ Configurado" if health.get('openai_configured') else "❌ Não configurado")
                        st.metric("Versão", health.get('version', 'N/A'))
                    with col_b:
                        st.metric("Ambiente", health.get('environment', 'N/A'))
                        timestamp = health.get('timestamp', 0)
                        if timestamp:
                            import datetime
                            dt = datetime.datetime.fromtimestamp(timestamp)
                            st.metric("Última verificação", dt.strftime("%H:%M:%S"))
                
                self.log_user_action("api_status_check", {"status": "healthy"})
            else:
                error_msg = health.get('error', 'Erro desconhecido')
                st.error(f"❌ API indisponível: {error_msg}")
                
                # Instruções específicas baseadas no erro
                if "conectar" in error_msg.lower() or "connection" in error_msg.lower():
...
docker-compose up
                ...
                self.log_user_action("api_status_check", {"status": "unhealthy", "error": error_msg})
    ...
    def render_results(self, result: Dict[str, Any]):
        ...
        # Dados extraídos e classificados
        extracted_data = result.get('extracted_data', {})
        classification_data = result.get('classification_data', {})
        ...
        # Safe formatting for processing_time
        try:
            safe_processing_time = float(processing_time or 0)
            time_display = f"{safe_processing_time:.2f}s"
        except (ValueError, TypeError):
            time_display = "N/A"
        st.success(f"✅ Documento processado e classificado com sucesso em {time_display}")
        st.markdown('</div>', unsafe_allow_html=True)
        ...
        with col2:
            data_emissao = extracted_data.get('data_emissao') or 'N/A'
            safe_data_emissao = str(data_emissao) if data_emissao is not None else 'N/A'
            st.metric(
                label="📅 Data Emissão",
                value=safe_data_emissao,
                delta=None,
                help="Data de emissão do documento"
            )
        
        with col3:
            cfop = extracted_data.get('cfop') or 'N/A'
            safe_cfop = str(cfop) if cfop is not None else 'N/A'
            st.metric(
                label="🏷️ CFOP",
                value=safe_cfop,
                delta=None,
        ...
        with col4:
            itens_count = len(extracted_data.get('itens', []))
            st.metric(
                label="📦 Itens",
                value=itens_count,
                delta=None,
        ...
        # Seção de Classificação Contábil
        if classification_data:
            st.markdown("## 🏷️ Classificação Contábil")
            ...
            with col_class1:
                ...
                conta_contabil = classification_data.get('conta_contabil') or 'N/A'
                st.markdown(f"**{conta_contabil}**")
                st.markdown('</div>', unsafe_allow_html=True)
            
            with col_class2:
                st.markdown('<div class="metric-card">', unsafe_allow_html=True)
                st.markdown("### 🏢 Centro de Custo")
                centro_custo = classification_data.get('centro_de_custo') or 'N/A'
                st.markdown(f"**{centro_custo}**")
                st.markdown('</div>', unsafe_allow_html=True)
            
            with col_class3:
                st.markdown('<div class="metric-card">', unsafe_allow_html=True)
                st.markdown("### 📝 Tipo de Lançamento")
                tipo_lancamento = classification_data.get('tipo_lancamento') or 'N/A'
                st.markdown(f"**{tipo_lancamento}**")
                st.markdown('</div>', unsafe_allow_html=True)
            
            # Justificativa da classificação
            justificativa = classification_data.get('justificativa') or ''
            if justificativa:
                st.markdown('<div class="info-box">', unsafe_allow_html=True)
        ...
        with tab1:
            self.render_general_data(extracted_data)
        
        with tab2:
            self.render_tax_data(extracted_data.get('impostos', {}))
        
        with tab3:
            self.render_items_data(extracted_data.get('itens', []))
        
        with tab4:
            self.render_classification_details(classification_data)
        
        with tab5:
            self.render_charts(extracted_data)
        
        with tab6:
            self.render_json_data({
                "extracted_data": extracted_data,
                "classification_data": classification_data
            })
    
    def render_classification_details(self, classification_data: Dict[str, Any]):
        """Renderiza detalhes da classificação contábil"""
        st.markdown("### 🏷️ Detalhes da Classificação Contábil")
        
        if not classification_data:
            st.info("ℹ️ Nenhuma informação de classificação disponível.")
            return
        ...
        with col1:
            ...
            classification_info = [
                ("Conta Contábil", classification_data.get('conta_contabil') or 'N/A'),
                ("Centro de Custo", classification_data.get('centro_de_custo') or 'N/A'),
                ("Tipo de Lançamento", classification_data.get('tipo_lancamento') or 'N/A'),
                ("Documento ID", classification_data.get('document_id') or 'N/A'),
                ("Tipo de Documento", classification_data.get('document_type') or 'N/A')
            ]
            
            for label, value in classification_info:
                # Ensure value is not None before formatting
                safe_value = str(value) if value is not None else 'N/A'
                st.markdown(f"**{label}:** {safe_value}")
        
        with col2:
            st.markdown("#### 💡 Análise da Classificação")
            ...
            if justificativa:
                ...
                st.info("Nenhuma justificativa fornecida.")
            
            # Sugestões de melhoria (se aplicável)
            st.markdown("**💡 Dicas para Classificação:**")
            st.markdown("""
            - A classificação é baseada em análise de IA
        ...
        # Botão para download da classificação
        if classification_data:
            classification_json = json.dumps(classification_data, ensure_ascii=False, indent=2)
            st.download_button(
                label="📥 Baixar Classificação (JSON)",
                data=classification_json,
                file_name=f"classificacao_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    ...
    def render_general_data(self, data: Dict[str, Any]):
        ...
        with col1:
            ...
            info_data = [
                ("Tipo", (data.get('documento') or 'N/A').upper()),
                ("Número", data.get('numero_documento') or 'N/A'),
                ("Série", data.get('serie') or 'N/A'),
                ("Moeda", data.get('moeda') or 'N/A'),
                ("Forma Pagamento", data.get('forma_pagamento') or 'N/A')
            ]
        ...
        with col2:
            ...
            emitente = data.get('emitente') or 'N/A'
            destinatario = data.get('destinatario') or 'N/A'
            
            # Ensure values are not None before formatting
            safe_emitente = str(emitente) if emitente is not None else 'N/A'
            safe_destinatario = str(destinatario) if destinatario is not None else 'N/A'
            
            st.markdown(f"**Emitente:** {safe_emitente}")
            st.markdown(f"**Destinatário:** {safe_destinatario}")
            
            st.markdown("### 🏷️ Classificação Fiscal")
            
            fiscal_data = [
                ("CFOP", data.get('cfop') or 'N/A'),
                ("NCM", data.get('ncm') or 'N/A'),
                ("CST", data.get('cst') or 'N/A')
            ]
    ...
    def render_items_data(self, itens: List[Dict[str, Any]]):
        ...
        with col2:
            # Safe formatting for total_value
            try:
                safe_total_value = float(total_value or 0)
                total_display = f"R$ {safe_total_value:,.2f}"
            except (ValueError, TypeError):
                total_display = "R$ 0,00"
            st.metric("Valor Total", total_display)
        
        with col3:
            # Safe formatting for avg_value
            try:
                safe_avg_value = float(avg_value or 0)
                avg_display = f"R$ {safe_avg_value:,.2f}"
            except (ValueError, TypeError):
                avg_display = "R$ 0,00"
            st.metric("Valor Médio", avg_display)
        ...
        if itens:
            ...
            for i, item in enumerate(itens, 1):
                ...
                # Safe description truncation - ensure descricao is a string
                if descricao and isinstance(descricao, str):
                    desc_display = descricao[:50] + ('...' if len(descricao) > 50 else '')
                else:
                    desc_display = str(descricao) if descricao else 'N/A'
        ...
        with col2:
            if itens and len(itens) > 1:
                ...
                for i, item in enumerate(itens[:10], 1):
                    ...
                    descricao = item.get('descricao') or 'N/A'
                    # Safe description truncation for chart - ensure descricao is a string
                    if descricao and isinstance(descricao, str):
                        desc_chart = descricao[:30] + ('...' if len(descricao) > 30 else '')
                    else:
                        desc_chart = str(descricao) if descricao else 'N/A'
